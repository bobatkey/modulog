...
module BTree (S : Imp_syntax.S) (P : BTREE_PARAMS with module S = S) : sig
  ...
  let iterate_range from upto tree body =
    ...
    begin%monoid
      ...
      loop begin%monoid
        ...
        ifthen (!i != !(!x #-> nkeys) || is_empty) break;
  ...
  let node_is_full x = !(x #-> nkeys) == const max_keys
    ...
    begin%monoid
      while_ (not !(!x #-> leaf)) begin%monoid
        ...
        ifthen (node_is_full !(!(!x #-> children) #@ !i))
          begin%monoid
            split_child !x !i;
            ifthen (key_lt !(!(!x #-> keys) #@ !i) key) (incr i)
          end;
        x:= !(!(!x #-> children) #@ !i)
      end;
